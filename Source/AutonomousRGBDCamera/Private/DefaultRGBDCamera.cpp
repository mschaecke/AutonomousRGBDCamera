...
class AUTONOMOUSRGBDCAMERA_API ADefaultRGBDCamera::PrivateData
{
public:
...
// Sets default values
ADefaultRGBDCamera::ADefaultRGBDCamera() /*: ACameraActor(), Width(960), Height(540), Framerate(1), FieldOfView(90.0), ServerPort(10000), FrameTime(1.0f / Framerate), TimePassed(0), ColorsUsed(0)*/
{
	...
	ConstructorHelpers::FObjectFinder<UMaterial> MaterialDepthFinder(TEXT("Material'/AutonomousRGBDCamera/SceneDepth.SceneDepth'"));
...
ADefaultRGBDCamera::~ADefaultRGBDCamera()
{
	delete Priv;
...
void ADefaultRGBDCamera::BeginPlay()
{
	Super::BeginPlay();
	...
	if (bCaptureColorImage)
	{
		...
		Priv->ThreadColor = std::thread(&ADefaultRGBDCamera::ProcessColor, this);
	}
	if (bCaptureDepthImage)
	{
		...
		bCompActive = true;
		Priv->ThreadDepth = std::thread(&ADefaultRGBDCamera::ProcessDepth, this);
	}
	if (bCaptureObjectMaskImage)
	{
		...
		bCompActive = true;
		Priv->ThreadObject = std::thread(&ADefaultRGBDCamera::ProcessObject, this);
	}
}

// Called when the game starts or when spawned
void ADefaultRGBDCamera::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void ADefaultRGBDCamera::Tick(float DeltaTime)
{
	...
	Priv->Buffer->StartWriting(ObjectToColor, ObjectColors, SceneGraph);
...
void ADefaultRGBDCamera::SetFramerate(const float _Framerate)
{
...
void ADefaultRGBDCamera::Pause(const bool _Pause)
{
	Paused = _Pause;
	if (Paused)
	{
...
bool ADefaultRGBDCamera::IsPaused() const
{
	return Paused;
}

void ADefaultRGBDCamera::ShowFlagsBasicSetting(FEngineShowFlags &ShowFlags) const
{
...
void ADefaultRGBDCamera::ShowFlagsLit(FEngineShowFlags &ShowFlags) const
{
...
void ADefaultRGBDCamera::ShowFlagsPostProcess(FEngineShowFlags &ShowFlags) const
{
	ShowFlagsBasicSetting(ShowFlags);
	ShowFlags.SetPostProcessing(true);
...
void ADefaultRGBDCamera::ShowFlagsVertexColor(FEngineShowFlags &ShowFlags) const
{
...
void ADefaultRGBDCamera::ReadImage(UTextureRenderTarget2D *RenderTarget, TArray<FFloat16Color> &ImageData) const
{
	FTextureRenderTargetResource *RenderTargetResource = RenderTarget->GameThread_GetRenderTargetResource();
	RenderTargetResource->ReadFloat16Pixels(ImageData);
}

void ADefaultRGBDCamera::ReadColorImage(UTextureRenderTarget2D *RenderTarget, TArray<FColor> &ImageData) const
{
...
void ADefaultRGBDCamera::ToColorImage(const TArray<FFloat16Color> &ImageData, uint8 *Bytes) const
{
...
void ADefaultRGBDCamera::ToColorRGBImage(const TArray<FColor> &ImageData, uint8 *Bytes) const
{
...
void ADefaultRGBDCamera::ToDepthImage(const TArray<FFloat16Color> &ImageData, uint8 *Bytes) const
{
	const FFloat16Color *itI = ImageData.GetData();
...
void ADefaultRGBDCamera::StoreImage(const uint8 *ImageData, const uint32 Size, const char *Name) const
{
 ...
 */
void ADefaultRGBDCamera::GenerateColors(const uint32_t NumberOfColors)
{
 ...
 *
 */
bool ADefaultRGBDCamera::ColorObject(AActor *Actor, const FString &name)
{
...
bool ADefaultRGBDCamera::ColorAllObjects()
{
...
void ADefaultRGBDCamera::RemoveNonExistingActorsFromColorMap()
{
...
void ADefaultRGBDCamera::ProcessColor()
{
	while(true)
	{
...
void ADefaultRGBDCamera::ProcessDepth()
{
	while(true)
	{
...
void ADefaultRGBDCamera::ProcessObject()
{
	while(true)
	{
